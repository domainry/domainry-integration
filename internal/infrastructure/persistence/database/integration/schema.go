...
func SchemaMigrations(driver, schema string) ([]modulehost.SchemaMigration, error) {
	...
	builders := []*ormbuilder.CreateTableBuilder{
...
func ownerIndexStatements(driver ormdialect.Name, renderer modulehost.Dialect) ([]string, error) {
	...
	for _, spec := range specs {
		builder := ormbuilder.NewCreateIndexBuilder(renderer, spec.name, spec.table).Columns(spec.columns...)
...
func table(renderer modulehost.Dialect, name string, columns ...ormbuilder.SchemaColumn) *ormbuilder.CreateTableBuilder {
	return ormbuilder.NewCreateTableBuilder(renderer, name).WithoutSystemColumns().IfNotExists().Columns(columns...).PrimaryKey("id")
}

func req(name string, kind ormbuilder.ColumnType) ormbuilder.SchemaColumn {
	return ormbuilder.DefineColumn(name, kind).NotNull()
}
func opt(name string, kind ormbuilder.ColumnType) ormbuilder.SchemaColumn {
	return ormbuilder.DefineColumn(name, kind)
}
func key(name string) ormbuilder.SchemaColumn     { return req(name, ormbuilder.TextKeyType(255)) }
func scope(name string) ormbuilder.SchemaColumn   { return req(name, ormbuilder.TextKeyType(191)) }
func text(name string) ormbuilder.SchemaColumn    { return req(name, ormbuilder.LongTextType()) }
func integer(name string) ormbuilder.SchemaColumn { return req(name, ormbuilder.BigIntType()) }
...
func definitionTable(renderer modulehost.Dialect, name string) *ormbuilder.CreateTableBuilder {
	return table(renderer, name,
		...
		opt("disabled_at", ormbuilder.TextKeyType(255)), key("created_at"), key("updated_at"),
	).Unique("resource_key")
}

func connectionsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_connections", key("id"), scope("connection_key"), scope("workspace_id"), scope("connector_key"), key("provider_key"), opt("name", ormbuilder.TextType()), key("status"), text("config_json"), text("secret_refs_json"), opt("created_by", ormbuilder.TextKeyType(255)), key("created_at"), key("updated_at")).Unique("workspace_id", "connection_key")
}

func providerStatesTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
...
func apiKeysTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_api_keys", key("id"), scope("api_key"), scope("workspace_id"), opt("name", ormbuilder.TextType()), key("token_prefix"), key("token_hash"), key("actor_id"), key("role_key"), text("scopes_json"), key("status"), key("expires_at"), key("last_used_at"), key("created_by"), key("created_at"), key("updated_at"), key("disabled_at")).Unique("workspace_id", "api_key").Unique("token_hash")
}

func secretMaterialsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
...
func secretsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_secrets", key("id"), scope("secret_key"), scope("workspace_id"), key("kind"), key("status"), opt("description", ormbuilder.TextType()), opt("value_ref", ormbuilder.TextType()), opt("fingerprint", ormbuilder.TextKeyType(255)), opt("created_by", ormbuilder.TextKeyType(255)), key("created_at"), key("updated_at"), opt("disabled_at", ormbuilder.TextKeyType(255)), key("expires_at"), key("rotated_at"), key("revoked_at"), key("last_tested_at"), key("last_test_status"), text("last_test_error")).Unique("workspace_id", "secret_key")
}

func externalIdentitiesTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_external_identities", key("id"), scope("identity_key"), scope("workspace_id"), key("provider"), scope("external_subject"), key("external_subject_type"), opt("external_name", ormbuilder.TextType()), opt("external_organization", ormbuilder.TextType()), opt("external_department", ormbuilder.TextType()), opt("external_group", ormbuilder.TextType()), opt("external_bot_id", ormbuilder.TextType()), key("actor_id"), key("role_key"), key("status"), opt("last_resolved_at", ormbuilder.TextKeyType(255)), key("created_by"), key("created_at"), key("updated_at"), opt("disabled_at", ormbuilder.TextKeyType(255))).Unique("workspace_id", "identity_key").Unique("workspace_id", "provider", "external_subject")
}

func invocationsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_invocations", key("id"), scope("workspace_id"), scope("connector_key"), key("provider_key"), opt("connection_key", ormbuilder.TextKeyType(191)), key("operation"), key("status"), integer("duration_ms"), opt("request_ref", ormbuilder.TextType()), opt("response_ref", ormbuilder.TextType()), opt("error", ormbuilder.TextType()), opt("event_id", ormbuilder.TextKeyType(255)), opt("object_key", ormbuilder.TextKeyType(255)), opt("record_id", ormbuilder.TextKeyType(255)), opt("workflow_execution_id", ormbuilder.TextKeyType(255)), text("metadata_json"), key("created_at"), key("updated_at")).Unique("workspace_id", "id")
}

func eventsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_events", key("id"), scope("workspace_id"), scope("provider"), scope("event_type"), scope("external_id"), key("status"), text("payload_json"), opt("error", ormbuilder.TextType()), integer("attempt_count"), key("next_retry_at"), key("last_attempt_at"), key("lease_owner"), key("lease_expires_at"), integer("fencing_token"), key("received_at"), key("updated_at")).Unique("workspace_id", "provider", "external_id")
}

func mappingIntentsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
...
func webhookNoncesTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
...
func webhookSubscriptionsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
	return table(r, "integration_webhook_subscriptions", key("id"), scope("subscription_key"), scope("workspace_id"), opt("name", ormbuilder.TextType()), scope("connector_key"), scope("connection_key"), text("event_types_json"), key("status"), opt("description", ormbuilder.TextType()), key("created_by"), key("created_at"), key("updated_at"), key("disabled_at")).Unique("workspace_id", "subscription_key")
}

func credentialRefreshLeasesTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
...
func webPushSubscriptionsTable(r modulehost.Dialect) *ormbuilder.CreateTableBuilder {
